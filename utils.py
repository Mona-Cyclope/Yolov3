...
def detect_yolo(net, meta, inpt, version="alexeyab", score_min=0.45, score_min_class_specific=None, classes_filter=None,
    ...
    try:
        ...
        __LOG.debug("boxes_final={}".format(boxes_final))
        return boxes_final
    except Exception as e:
        raise Exception("Failed to apply YOLO detection") from e


def import_darknet(version="alexeyab"):
    try:
        if version in ["3", 3, "v3"]:
            __LOG.debug("Importing library darknet for YOLO v3")
            ...
            __LOG.debug("Importing library darknet for YOLO v2")
            # sys.path.insert(0, "/cyclope/cyclope-eye/deps/darknet/python/")
            global dn2
            import utils.darknet2 as dn2
        elif version == "alexeyab":
            __LOG.debug("Importing library darknet for YOLO AlexeyAB")
            sys.path.insert(0, os.getenv("DARKNET3_PATH", "/cyclope/share/deps/darknet-alexeyab/"))
            global dnab
            # import utils.darknet as dnab
            import darknet as dnab
    except Exception as e:
        raise Exception("Failed to load darknet") from e
    return

def load_yolo(dict_path, version="alexeyab", gpu_id=0):
    try:
        __LOG.debug("Loading YOLO model")
        if version == "alexeyab":
        ...
        if dict_path.get("meta"):
            __LOG.debug("meta path={}".format(dict_path.get("meta")))
            meta = load_meta(dict_path.get("meta").encode('utf-8'))
        else:
            __LOG.debug("No YOLO meta path")
            meta = None

        res_dict = {"net": net,
                    "meta": meta}
        __LOG.debug("YOLO Model loaded - {}".format(res_dict))

        return res_dict
    except Exception as e:
        raise Exception("Failed to load YOLO model") from e


def yolo(net, meta, frame, score_min=0.45, version="alexeyab"):
    try:
        __LOG.debug("Detecting objects with YOLO and reformatting coordinates")
...
def detect_objects(net, meta, image, score_min=.45, hier_thresh=.5, nms=.45, version="alexeyab", verbose=False):
    if verbose:
        __LOG.debug("Detecting objects based on model {}".format(net))
    ...
    if verbose:
        __LOG.debug("Detected {} objects".format(str(count)))
    return boxes, probs


def reformat_boxes(net, meta, boxes, probs):
    __LOG.debug("Reformatting boxes coordinates")
    boxes_reformatted = []
    num = dn2.num_boxes(net)
    for j in range(num):
        for i in range(meta.classes):
